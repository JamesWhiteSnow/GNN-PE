...
delete_folder(dataset_path+'gnn-pge')
create_folder(dataset_path+'gnn-pge')

with open(dataset_path+'gnn-pge/membership.txt','w') as f:
    for node,degree in nodes_degree_sorted:
        f.write(str(node)+' '+str(membership[node])+'\n')

delete_folder(dataset_path+'gnn-pge/partitions')
create_folder(dataset_path+'gnn-pge/partitions')
for i in range(partition_num):
    create_folder(dataset_path+'gnn-pge/partitions/'+'partition-{0}'.format(i))